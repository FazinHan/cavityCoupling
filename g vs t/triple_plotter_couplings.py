...
filename = 'combined_plots_params_erred.csv'
...
err_arr = np.full_like(g1, errors)

assert len(g1) == len(g2) == len(t) == len(err_arr), "Data arrays must have the same length"
...
axs[0].errorbar(t, g1, yerr=err_arr, fmt='none',ecolor='r',capsize=10)#, markersize=15, capsize=5)
...
# Customize plot
axs[0].set_xlabel('t (mm)',fontsize=labelsize)
axs[0].set_ylabel('$g$ (GHz)',fontsize=labelsize)
...
axs[0].text(.005,.3,'(a)',fontsize=labelsize)
...
axs[2].errorbar(g2, g1, xerr=err_arr, yerr=err_arr, fmt='none', ecolor='g', capsize=10)#, markersize=15, capsize=5)
axs[2].plot(g2, slope * g2 + intercept, 'g-', label=f'Fit: $g_{{Py}} = {slope:.2f} g_{{YIG}} + {intercept:.2f}$')
axs[2].set_xlabel("$g_2$ (GHz)",fontsize=labelsize)
axs[2].set_ylabel("$g_1$ (GHz)",fontsize=labelsize)
...
axs[2].text(-.03,.255,'(c)',fontsize=labelsize)
plt.tight_layout()
...
axs[1].set_xlabel('t (mm)',fontsize=labelsize)
axs[1].set_ylabel('$g_2$ (GHz)',fontsize=labelsize)
# plt.grid()
